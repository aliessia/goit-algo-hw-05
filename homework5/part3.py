...
def measure_time(func, text, pattern):
    setup_code = f"from __main__ import {func.__name__} as tested_func"
    stmt = "tested_func(text, pattern)"
    return min(timeit.repeat(setup=setup_code, stmt=stmt, repeat=3, number=100, globals=locals()))
...
for text, name in [(text1, "Article 1"), (text2, "Article 2")]:
    ...
    for func in [boyer_moore_search, kmp_search, rabin_karp_search]:
        pattern_to_use = pattern_exist_1 if name == "Article 1" else pattern_exist_2
        time_exist = measure_time(func, text, pattern_to_use)
        pattern_to_use = pattern_fiction_1 if name == "Article 1" else pattern_fiction_2
        time_fiction = measure_time(func, text, pattern_to_use)
